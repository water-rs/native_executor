//! Cross-thread safe values with main-thread execution guarantees.
//!
//! `MainValue<T>` enables sharing values across threads while ensuring all access
//! and drop operations occur on the main thread via async scheduling.
//!
//! ```rust
//! use native_executor::MainValue;
//!
//! let ui_data = MainValue::new(String::from("Button"));
//! # async {
//! let len = ui_data.handle(|s| s.len()).await;  // Executes on main thread
//! # };
//! ```
